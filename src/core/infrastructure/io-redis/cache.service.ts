...
export class CacheService {
  ...
  generateCacheKey(prefix: string, obj: Record<string, unknown>) {
    const hash = createHash('sha256').update(this.stringify(obj)).digest('hex');
    const cacheKey = `${prefix}:${hash}`;
    return cacheKey;
  }

  private stringify(data: unknown): string {
    return JSON.stringify(data, (key, val: unknown) => {
  ...
  private parse(data: string): unknown {
    return JSON.parse(data, (_key, val: unknown) => {
      if (val && typeof val === 'object' && '__type' in val && 'value' in val) {
        const obj = val as { __type: string; value: string };
